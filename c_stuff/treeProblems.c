...
struct node* extract_leaves(struct node* root,struct node** headRef)
{
	...
	root->right=(root->right,*headRef);
	root->left=(root->left,*headRef);
...
struct node* remove_nodes(struct node* root,int k)
{
	...
	if(root->val > k)
		return root;
	left=remove_nodes(root->left,k - root->val);
	right=remove_nodes(root->right,k - root->val);
...
struct node* pruneUtil(struct node* root,int k,int* sum)
{
	...
	root->left=(root->left,k,sum);
	root->right=(root->right,k,sum);
...
int check_leaf_level(struct node* root,int level)
{
	...
	if(root->left==null && root->right==null)
		return level+1;
	return check_leaf_level(node->left,level+1)==check_leaf_level(node->right,level+1);
}
...
int get_level_order_difference(struct node* root)
{
	if(root==null)
		return 0;
	return root-data - get_level_order_difference(root->left) - get_level_order_difference(root->right);
}
...
void sum(struct node* root,int level,int *osum,int* esum)
{
	...
	if(Level%2==0)
		...
		*osum+=root->val;
	sum(node->left,level+1,osum,esum);
	sum(node->right,level+1,osum,esum);
}
...
struct node* bintree2list(node *root)
{
    // Base case
    if (root == NULL)
        return root;
